...
Given('that I am authenticated as a user with role {string}', async function (role) {
  ...
  if (response.status === 200) {
    ...
    this.token = jwt.sign({ sub: email, role: role }, secret, { expiresIn: '1h' });
  }
});

When('I make a {string} request to {string} without a token', async function (method, path) {
  this.token = null;
...
When('I make a {string} request to {string} with a valid employee', async function (method, path) {
